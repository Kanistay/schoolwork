...
-- GlobalLoadCfg fonksiyonu - LoadCfg'nin alternatifi
local function GlobalLoadCfg(Config)
	pcall(function()
		local Data = SafeJsonDecode(Config)
		for a, b in pairs(Data) do
			if OrionLib.Flags[a] then
...
function OrionLib:MakeWindow(WindowConfig)
	...
	RunService.RenderStepped:Connect(function(deltaTime)
		-- Performans optimizasyonu için güncelleme sıklığını sınırla
		lastUpdate = lastUpdate + deltaTime
		if lastUpdate < updateInterval then return end
		lastUpdate = 0
		
		pcall(function()
			-- Artı butonunun konumunu MainWindow'un sol altına ayarla
	...
	function TabFunction:MakeTab(TabConfig)
		...
		local function GetElements(ItemParent)
			...
			function ElementFunction:AddSlider(SliderConfig)
				...
				-- Slider dolgu (drag edilebilir kısım)
				local SliderDrag = SetProps(MakeElement("RoundFrame", SliderConfig.Color, 0, 5), {
...
-- SaveCfg fonksiyonunu GlobalSaveCfg olarak değiştir ve yeni versiyonla çakışmayı önle
local function GlobalSaveCfg(Name)
	if not OrionLib.SaveCfg or not OrionLib.Folder then return end
...
-- GlobalLoadCfg fonksiyonu başa taşındı
