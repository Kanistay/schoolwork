...
function OrionLib:MakeWindow(WindowConfig)
	...
	AddConnection(MinimizeBtn.MouseButton1Up, function()
		if Minimized then
			...
			-- PlusBtn ve KanistayLabel'ı göster
			if ButtonContainer then ButtonContainer.Visible = true end
			if MainWindow:FindFirstChild("KanistayLabel") then MainWindow.KanistayLabel.Visible = true end
		else
			-- Küçültme işlemi
...
local function setupOrionVisibilitySync()
    ...
    -- SnowContainer'ın parent frame'ini bul
    local targetFrame = nil
    
    -- SnowContainer'ı bul
    local snowContainer = orionScreenGui:FindFirstChild("SnowContainer", true) -- true ile recursive arama
    
    if snowContainer and snowContainer.Parent ~= orionScreenGui then
        -- SnowContainer'ın parent'ı Orion değilse, o zaman parent'ı hedef frame'dir
        targetFrame = snowContainer.Parent
        if not targetFrame:IsA("Frame") then
            warn("SnowContainer'ın parent'ı bir Frame değil!")
            return
        end
    else
        warn("SnowContainer bulunamadı veya doğrudan Orion'un çocuğu!")
        return
    end
    
    -- ButtonContainer ve KanistayLabel'ı bul
    ...
    local function syncVisibility()
        ...
        print("Visibility güncellendi: " .. tostring(isVisible))
    end
    
    -- İlk senkronizasyonu yap
    syncVisibility()
    
    -- Changed sinyali ile bağlantı kur
    local connection = targetFrame:GetPropertyChangedSignal("Visible"):Connect(syncVisibility)
    
    print("Orion visibility sync kuruldu!")
    
    -- Connection'ı geri döndür (isteğe bağlı olarak bağlantıyı kesebilmek için)
    return connection
end

-- Fonksiyonu çağır
local visibilityConnection = setupOrionVisibilitySync()

-- Bağlantıyı kesmek için (isteğe bağlı):
