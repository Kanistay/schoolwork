...
KanistayLabel.Visible = false -- Başlangıçta gizli, updateComponentsVisibility ile kontrol edilecek
KanistayLabel.Parent = Orion
	...
	-- Görünürlüğü tek bir fonksiyonla güncelle
	pcall(updateComponentsVisibility)
	...
	if Minimized then
		...
		-- Minimized durumunu güncelle ve görünürlüğü ayarla
		Minimized = false
		pcall(updateComponentsVisibility)
	else
		-- Küçültme işlemi
...
ButtonContainer.Visible = false -- Başlangıçta gizli, updateComponentsVisibility ile kontrol edilecek
...
-- Yeni fonksiyon: KanistayLabel ve ButtonContainer görünürlüğünü kontrol eden tek bir fonksiyon
local function updateComponentsVisibility()
    pcall(function()
        if not (MainWindow and MainWindow.Parent) then return end
        local shouldBeVisible = MainWindow.Visible and not Minimized and not UIHidden
        local orionUI = MainWindow.Parent
        local kanistayLabel = orionUI and orionUI:FindFirstChild("KanistayLabel")
        local buttonContainer = orionUI and orionUI:FindFirstChild("ButtonContainer")
        
        print("kanistayLabel:", kanistayLabel, "parent:", kanistayLabel and kanistayLabel.Parent)
        print("buttonContainer:", buttonContainer, "parent:", buttonContainer and buttonContainer.Parent)
        
        if kanistayLabel and kanistayLabel.Parent and kanistayLabel:IsA("GuiObject") then
            kanistayLabel.Visible = shouldBeVisible
        end
        if buttonContainer and buttonContainer.Parent and buttonContainer:IsA("GuiObject") then
...
-- ButtonContainer'ı Orion ScreenGui'sinden bul ve child ekle
local orionScreenGui
repeat
    orionScreenGui = game:GetService("CoreGui"):FindFirstChild("Orion") or game.Players.LocalPlayer.PlayerGui:FindFirstChild("Orion")
    if not orionScreenGui then
        task.wait(0.1)
    end
until orionScreenGui

local buttonContainer
repeat
    buttonContainer = orionScreenGui:FindFirstChild("ButtonContainer")
    if not buttonContainer then
        task.wait(0.1)
    end
until buttonContainer
...
ButtonContainer = buttonContainer -- global değişkeni güncelle
