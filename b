...
local function MakeElement(ElementName, ...)
	...
	elseif ElementName == "RoundFrame" then
		...
		local Corner = Create("UICorner", {
			CornerRadius = UDim.new(0, 10) -- Daha yumuşak köşeler (4'ten 10'a çıkarıldı)
		})
		Corner.Parent = Frame
...
function OrionLib:MakeWindow(WindowConfig)
	...
	local MainWindow = AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(255, 255, 255), 0, 20), {
		Parent = Orion,
	...
	-- Buton tıklama olayı - ScrollingFrame'i göster/gizle
	TabsToggleBtn.MouseButton1Click:Connect(function()
		...
		-- WindowStuff ve TabHolder boyutlandırması
		pcall(function()
			if isTabsVisible then
				-- ScrollingFrame'i göster
				if TabHolder then TabHolder.Visible = true end
				
				-- WindowStuff'ı normal boyuta getir (fail-safe boyutlandırma)
				if WindowStuff then
					TweenService:Create(WindowStuff, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {
						Size = UDim2.new(0, 150, 1, -50),
						Position = UDim2.new(0, 0, 0, 50) -- Sabit pozisyon değeri
					}):Play()
				...
				-- ItemContainer boyutlandırma - ana fonksiyonalite
				if MainWindow then
				...
				-- WindowStuff'ı küçült (fail-safe boyutlandırma)
				if WindowStuff then
					-- Önce pozisyonu düzelt, sonra boyutu değiştir
					WindowStuff.Position = UDim2.new(0, 0, 0, 0)
					TweenService:Create(WindowStuff, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {
						Size = UDim2.new(0, 30, 1, -50)
					}):Play()
				...
				-- ItemContainer genişletme - ana fonksiyonalite
				if MainWindow then
					for _, ItemContainer in pairs(MainWindow:GetChildren()) do
						if ItemContainer.Name == "ItemContainer" and ItemContainer.Visible then
							pcall(function()
	...
	-- Ultra-optimize edilmiş kar efekti oluşturma fonksiyonu
	local function CreateSnowEffect()
		...
		local maxSnowflakes = 12 -- Daha az kar tanesi (15'ten 12'ye düşürüldü)
		...
		local fpsTarget = 60 -- Hedef FPS
		
		-- Kar tanesi havuzu - performans için önceden oluşturulmuş kar taneleri
		local snowflakePool = {}
		local poolSize = 20
		
		-- Kar tanesi havuzunu oluştur
		local function createSnowflakePool()
			for i = 1, poolSize do
				local snowflake = Instance.new("Frame")
				snowflake.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
				snowflake.BorderSizePixel = 0
				snowflake.Size = UDim2.new(0, math.random(2, 3), 0, math.random(2, 3))
		...
		-- Kar tanesi oluşturma fonksiyonu - havuz kullanarak ultra optimize edilmiş
		local function createSnowflake(extraSnow)
			if activeSnowflakes >= maxSnowflakes and not extraSnow then return end
			
			-- Havuzdan kullanılmayan bir kar tanesi bul
			local snowflake = nil
			for _, flake in ipairs(snowflakePool) do
				if not flake.Visible then
					snowflake = flake
					break
				end
			end
			
			-- Havuzda boş kar tanesi yoksa yeni oluşturma
			if not snowflake then return end
			
			-- Kar tanesini yeniden kullan
			snowflake.BackgroundTransparency = math.random(0.3, 0.7) -- Daha şeffaf
			...
			-- Kar tanesinin düşüş animasyonu - ultra optimize edilmiş
			spawn(function()
				-- Daha gerçekçi kar tanesi hareketi için değişkenler
				local duration = math.random(6, 10) -- Daha yavaş düşüş (5-8'den 6-10'a)
				local startTime = tick()
				local startX = snowflake.Position.X.Scale
				local amplitude = math.random(3, 8) / 100 -- Sallanma miktarı
				local frequency = math.random(5, 10) / 10 -- Sallanma hızı
				
				-- Ekstra kar tanesi mi kontrolü - ekstralar daha hızlı düşsün
				if extraSnow then
					duration = math.random(4, 7) -- Ekstra kar taneleri daha hızlı düşer
					amplitude = math.random(5, 10) / 100 -- Ve daha fazla sallanır
				end
				
				-- Kar tanesi boyutunu rastgele ayarla (daha gerçekçi)
				local size = math.random(2, 3)
				snowflake.Size = UDim2.new(0, size, 0, size)
				
				-- Kar tanesi düşüş döngüsü - performans için optimize edildi
				while snowflake and snowflake.Parent and snowflake.Visible and (snowflake.Position.Y.Scale < 1) do
					local elapsed = tick() - startTime
					local progress = elapsed / duration
					
					-- Yatay sallanma hareketi - daha doğal görünüm için
					...
					wait(isLowPerformance and 0.08 or 0.05)
				end
				
				-- Kar tanesi ekrandan çıktığında havuza geri döndür (yok etmek yerine)
		...
		local function checkPerformance()
			...
			-- Her 3 saniyede bir performans kontrolü
			if frameTimeTotal >= 3 then
				local averageFrameTime = frameTimeTotal / frameCount
				
				-- 60 FPS'den düşükse (16.67ms'den yüksek frame süresi)
				if averageFrameTime > 0.0167 then
					isLowPerformance = true
					maxSnowflakes = 8 -- Düşük performansta çok daha az kar tanesi
				else
					isLowPerformance = false
					maxSnowflakes = 15 -- Normal performansta standart kar tanesi sayısı
				end
		...
		-- Menü sallanma algılama fonksiyonu
		local function checkMenuShaking()
			local currentPosition = MainWindow.Position
			local currentVector = Vector2.new(currentPosition.X.Offset, currentPosition.Y.Offset)
			local movement = (currentVector - lastPosition).Magnitude
			
			-- Eğer belirli bir eşikten fazla hareket varsa, menü sallanıyor demektir
			if movement > 5 then
				isShaking = true
				lastShakeTime = tick()
				
				-- Menü sallanınca ekstra kar taneleri oluştur (FPS'e göre kontrollü)
				if not isLowPerformance then
					local extraSnowCount = math.min(5, math.floor(movement / 3))
					for i = 1, extraSnowCount do
						createSnowflake(true) -- Ekstra kar tanesi oluştur
					end
				end
			elseif isShaking and (tick() - lastShakeTime) > 0.5 then
				isShaking = false
			end
			
			lastPosition = currentVector
		end
		
		-- Düzenli aralıklarla kar taneleri oluştur - optimize edilmiş
		spawn(function()
			while MainWindow and MainWindow.Parent do
				-- Performans kontrolü
				checkPerformance()
				
				-- Menü sallanma kontrolü - her framede kontrol et
				checkMenuShaking()
				
				-- Menü görünür olduğunda kar tanelerini oluştur
				if MainWindow.Visible and not UIHidden then
					createSnowflake()
				end
				
				-- Performansa göre bekleme süresini ayarla - daha uzun bekleme süreleri
				local waitTime = isLowPerformance 
					and math.random(8, 12) / 10  -- Düşük performansta 0.8-1.2 saniye
					or math.random(5, 10) / 10   -- Normal performansta 0.5-1.0 saniye
	...
	-- WindowStuff'ın doğru konumunu ayarlayan fonksiyon
	local function FixWindowStuffPosition()
		if WindowStuff then
			if Minimized then
				...
				-- Normal durumda - tam sabit değerler kullan
	...
	ButtonContainer.Size = UDim2.new(0, 22, 0, 22) -- Daha küçük boyut (35x35'ten 22x22'ye)
	ButtonContainer.BackgroundTransparency = 1
	ButtonContainer.Visible = true
	ButtonContainer.ZIndex = 10000 -- En üstte görünmesi için
	ButtonContainer.Position = UDim2.new(0, 15, 1, -30) -- Başlangıç pozisyonu daha yukarıda
	...
	PlusBtn.BackgroundColor3 = Color3.fromRGB(30, 30, 30) -- Daha koyu arkaplan
	PlusBtn.BackgroundTransparency = 0.1
	PlusBtn.Text = "+"
	PlusBtn.TextColor3 = Color3.fromRGB(255, 255, 255) -- Beyaz yazı
	PlusBtn.Font = Enum.Font.GothamBold
	PlusBtn.TextSize = 14 -- Daha küçük yazı (20'den 14'e)
	PlusBtn.ZIndex = 10001 -- En üstte görünmesi için
		...
		UserInputService.InputChanged:Connect(function(input)
			if draggingTrans and input.UserInputType == Enum.UserInputType.MouseMovement and TransSliderBar then
				pcall(function()
					...
					if MainWindow then
						local tval = rel * 0.8 -- 0 ila 0.8 arası transparanlık
					...
					if TransValueLabel then
						TransValueLabel.Text = math.floor(rel * 100) .. "%"
					end
				end)
			end
		end)
	...
	function TabFunction:MakeTab(TabConfig)
		...
		local function GetElements(ItemParent)
			...
			function ElementFunction:AddDropdown(DropdownConfig)
				...
				-- Köşeleri yuvarla
				local Corner = MakeElement("Corner")
