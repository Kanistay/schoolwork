...
function OrionLib:MakeWindow(WindowConfig)
	...
	-- Kanistay HUB yazısı - ana menüye bitişik
	local KanistayLabel = Instance.new("TextLabel")
	KanistayLabel.Name = "KanistayLabel"
	KanistayLabel.Size = UDim2.new(0, 30, 0.8, 0) -- Yükseklik ana pencereye göre ayarlanacak
	KanistayLabel.Position = UDim2.new(0, -5, 0.5, 0) -- Menünün sol kenarına bitişik
	KanistayLabel.AnchorPoint = Vector2.new(1, 0.5)
	KanistayLabel.BackgroundTransparency = 1 -- Arka plan yok
	KanistayLabel.TextColor3 = Color3.fromRGB(255, 255, 255) -- Beyaz yazı
	KanistayLabel.Font = Enum.Font.GothamBold
	KanistayLabel.TextSize = 16
	...
	KanistayLabel.Rotation = 90 -- Dikey yazı (aşağıdan yukarı)
	KanistayLabel.ZIndex = 9999
	KanistayLabel.Parent = MainWindow
	
	-- Yazının yanıp sönme efekti ve pozisyon güncellemesi
	spawn(function()
		local visible = true
		while wait(0.8) do -- 0.8 saniyede bir değişim
			if MainWindow and MainWindow.Parent then
				...
				if KanistayLabel and KanistayLabel.Parent then
					-- Menü görünür ve küçültülmemiş ise
					if MainWindow.Visible and not Minimized and not UIHidden then
						KanistayLabel.TextTransparency = visible and 0.2 or 1.0 -- Görünür/görünmez arası geçiş
						
						-- Ana pencere yüksekliğine göre KanistayLabel'ı ayarla
						local height = MainWindow.AbsoluteSize.Y
						KanistayLabel.Size = UDim2.new(0, 120, 0, height * 0.8)
					else
						KanistayLabel.TextTransparency = 1.0 -- Tamamen şeffaf (görünmez)
					end
				end
			else
				break
	...
	AddConnection(MainWindow:GetPropertyChangedSignal("Size"), function()
		if WindowStuff and MainWindow and MainWindow:FindFirstChild("TopBar") then
			if not Minimized then
				...
				-- Kanistay HUB yazısının boyutunu pencere yüksekliğine göre ayarla
				if MainWindow:FindFirstChild("KanistayLabel") then
					local height = MainWindow.AbsoluteSize.Y
					KanistayLabel.Size = UDim2.new(0, 30, 0.8, 0)
					KanistayLabel.Position = UDim2.new(0, -5, 0.5, 0)
				end
			end
		end
	end)
	...
	AddConnection(CloseBtn.MouseButton1Up, function()
		-- PlusBtn ve KanistayLabel'ı gizle
		if ButtonContainer then ButtonContainer.Visible = false end
		if MainWindow:FindFirstChild("KanistayLabel") then MainWindow.KanistayLabel.Visible = false end
	...
	AddConnection(UserInputService.InputBegan, function(Input)
		if Input.KeyCode == Enum.KeyCode.RightShift and UIHidden then
			...
			-- PlusBtn ve KanistayLabel'ı göster (eğer küçültülmüş değilse)
			if not Minimized then
				if ButtonContainer then ButtonContainer.Visible = true end
				if MainWindow:FindFirstChild("KanistayLabel") then MainWindow.KanistayLabel.Visible = true end
			else
				-- Küçültülmüşse kesinlikle gizli kalsınlar
				if ButtonContainer then ButtonContainer.Visible = false end
				if MainWindow:FindFirstChild("KanistayLabel") then MainWindow.KanistayLabel.Visible = false end
			end
		end
	...
	local function CreateSnowEffect()
		...
		spawn(function()
			while MainWindow and MainWindow.Parent do
				...
				if MainWindow.Visible and not UIHidden and timeSinceLastSnowflake >= minSpawnInterval then
					createSnowflake()
	...
	AddConnection(MinimizeBtn.MouseButton1Up, function()
		if Minimized then
			...
			-- PlusBtn ve KanistayLabel'ı göster - sadece menü görünürse
			if not UIHidden then
				if ButtonContainer then ButtonContainer.Visible = true end
				if MainWindow:FindFirstChild("KanistayLabel") then 
					MainWindow.KanistayLabel.Visible = true 
			...
			-- PlusBtn ve KanistayLabel'ı kesinlikle gizle
			if ButtonContainer then ButtonContainer.Visible = false end
			if MainWindow:FindFirstChild("KanistayLabel") then MainWindow.KanistayLabel.Visible = false end
	...
	ButtonContainer.Position = UDim2.new(0, 15, 1, -90) -- Başlangıç pozisyonu (5 birim daha aşağı)
