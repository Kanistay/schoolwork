...
function OrionLib:MakeWindow(WindowConfig)
	...
	local MainWindow = AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(255, 255, 255), 0, 20), {
		...
		-- Sol kenara Kanistay HUB yazısı
		SetProps(MakeElement("TFrame"), {
			Size = UDim2.new(0, 20, 1, 0),
			Position = UDim2.new(0, 0, 0, 0),
			BackgroundTransparency = 1,
			Name = "SideTextContainer",
			ZIndex = 50
		}),
		
		-- Kar efekti için container
		SetProps(MakeElement("TFrame"), {
			Size = UDim2.new(1, 0, 1, 0),
			BackgroundTransparency = 1,
			Name = "SnowContainer",
			ZIndex = 0 -- Arkaplanda kalması için
		}),
	...
	TabsToggleBtn.Position = UDim2.new(0, 5, 0, 5) -- Sabit pozisyon (sol üst köşeye yakın)
	TabsToggleBtn.AnchorPoint = Vector2.new(0, 0)
	...
	-- WindowStuff boyut güncellemesi doğru konumda (MainWindow tanımlandıktan sonra)
	AddConnection(MainWindow:GetPropertyChangedSignal("Size"), function()
		if WindowStuff and MainWindow and MainWindow:FindFirstChild("TopBar") then
			WindowStuff.Size = UDim2.new(0, 150, 1, -(MainWindow.TopBar.Size.Y.Offset)) 
		end
	end)
	...
	local function CreateSnowEffect()
		...
		local maxSnowflakes = 20 -- Daha az kar tanesi (30'dan 20'ye düşürüldü)
		...
		local function createSnowflake()
			...
			-- Rastgele kar tanesi tipi seçimi
			local snowflakeType = math.random(1, 10)
			local snowflake
			
			if snowflakeType <= 8 then -- %80 normal kar tanesi
			...
			else -- %20 yıldız şeklinde kar tanesi
				snowflake = Instance.new("ImageLabel")
				snowflake.BackgroundTransparency = 1
				snowflake.Image = "rbxassetid://7228008715" -- Yıldız şeklinde ikon
				
				-- Farklı boyutlarda kar taneleri
				local size = math.random(4, 7)
				snowflake.Size = UDim2.new(0, size, 0, size)
				snowflake.ImageColor3 = Color3.fromRGB(220, 240, 255) -- Hafif mavi tonu
				snowflake.Rotation = math.random(0, 360) -- Rastgele rotasyon
			end
			...
			-- Şeffaflık ekle
			if snowflake:IsA("Frame") then
				snowflake.BackgroundTransparency = math.random(0, 0.6)
			else
				snowflake.ImageTransparency = math.random(0, 0.6)
			end
			
			snowflake.ZIndex = 100 -- En üstte görünmesi için yüksek ZIndex
			snowflake.Parent = SnowContainer
			
			table.insert(snowflakes, snowflake)
			activeSnowflakes = activeSnowflakes + 1
			
			-- Kar tanesinin düşüş animasyonu
			spawn(function()
				local duration = math.random(4, 8)
				local startTime = tick()
				local startX = snowflake.Position.X.Scale
				local amplitude = math.random(5, 15) / 100 -- Sallanma genişliği
				local frequency = math.random(10, 20) / 10 -- Sallanma hızı
		...
		local function checkPerformance()
			...
			-- Her 5 saniyede bir performans kontrolü
			if frameTimeTotal >= 5 then
				...
				if averageFrameTime > 0.0167 then
					...
					maxSnowflakes = 10 -- Düşük performansta daha az kar tanesi
				else
					isLowPerformance = false
					maxSnowflakes = 20 -- Normal performansta standart kar tanesi sayısı
				end
		...
		-- Düzenli aralıklarla kar taneleri oluştur
		spawn(function()
			while MainWindow and MainWindow.Parent do
				...
				-- Performansa göre bekleme süresini ayarla
				local waitTime = isLowPerformance 
					and math.random(5, 10) / 10  -- Düşük performansta 0.5-1.0 saniye
					or math.random(2, 8) / 10    -- Normal performansta 0.2-0.8 saniye
	...
	-- Sol kenara dikey "Kanistay HUB" yazısı ekleme
	local function AddSideText()
		local sideContainer = MainWindow:FindFirstChild("SideTextContainer")
		if not sideContainer then return end
		
		local sideText = Instance.new("TextLabel")
		sideText.BackgroundTransparency = 1
		sideText.Size = UDim2.new(0, 200, 0, 20)
		sideText.Position = UDim2.new(0, 10, 0.5, 0)
		sideText.AnchorPoint = Vector2.new(0, 0.5)
		sideText.Rotation = 90 -- Dikey yazı için 90 derece döndür
		sideText.Font = Enum.Font.GothamBold
		sideText.TextSize = 14
		sideText.TextColor3 = Color3.fromRGB(255, 255, 255)
		sideText.Text = "Kanistay HUB"
		sideText.TextTransparency = 0.2
		sideText.ZIndex = 50
		sideText.Parent = sideContainer
		
		-- Yazıya gölge efekti ekle
		...
		shadow.Text = "Kanistay HUB"
		shadow.TextTransparency = 0.5
		shadow.ZIndex = 49
		shadow.Parent = sideText
		
		-- Yazının boyut değişikliğinde konumunu koruma
		AddConnection(MainWindow:GetPropertyChangedSignal("Size"), function()
			if sideText and sideText.Parent then
				sideText.Position = UDim2.new(0, 10, 0.5, 0)
			end
		end)
	end
	
	-- Sol kenar yazısını ekle
	AddSideText()
	...
	AddConnection(MinimizeBtn.MouseButton1Up, function()
		if Minimized then
			...
			-- WindowStuff pozisyonunu düzelt ve görünür yap
			if WindowStuff then
				WindowStuff.Position = UDim2.new(0, 0, 0, 50) -- Doğru pozisyonu ayarla
			...
			-- Başlık genişliği ölçümünü daha doğru yapalım
			...
			-- Yakala ve sadece genişliği küçült
			...
			-- Küçültüldüğünde pozisyon düzeltmesi
			if WindowStuff then
				WindowStuff.Position = UDim2.new(0, 0, 0, 0) -- Pozisyonu sıfırla, taşmayı önle
			end
	...
	ButtonContainer.Size = UDim2.new(0, 25, 0, 25) -- Daha küçük boyut (35x35'ten 25x25'e)
	ButtonContainer.BackgroundTransparency = 1
	ButtonContainer.Visible = true
	ButtonContainer.ZIndex = 10000 -- En üstte görünmesi için
	ButtonContainer.Position = UDim2.new(0, 15, 1, -40) -- Başlangıç pozisyonu
	...
	PlusBtn.TextSize = 16 -- Daha küçük yazı (20'den 16'ya)
